...
while True:
    ...
    for entry in data.values():
        if 'heartRate' in entry and 'readingNumber' in entry:
            hr_entries.append((entry['readingNumber'], entry['heartRate']))
    
    hr_entries.sort(key=lambda x: x[0])
    last_entries = hr_entries[-25:]
    ...
    if len(hr_array) >= 12:
        try:
            features = hr_to_hrv_freq_features(hr_array, sampling_rate_hz=1/5)
